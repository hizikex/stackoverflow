...
export const updateTag: RequestHandler = async (req, res, next): Promise<void> => {
    try {
        const validTagUpdateInput = validateTagUpdateInput(req.body)
        const response = await tagController.processUpdateTag(validTagUpdateInput, req.params.tagName);
