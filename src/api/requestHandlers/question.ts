...
} from '../../core/validations/question';
...
export const ListQuestions: RequestHandler = async (
  req,
  res,
  next,
): Promise<void> => {
  try {
    ...
    const response =
      await questionController.processListQuestions(validQuestionList);
...
export const deleteQuestion: RequestHandler = async (
  ...
  try {
    const response = await questionController.processDeleteQuestion(
