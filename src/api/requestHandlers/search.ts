...
export const searchQuestionOrAnswer: RequestHandler = async (req, res, next): Promise<void> => {
    try {
        const validateSearchInput = validateQuestionAnswerQuery(req.query);
        const response = await searchController.processQuestionAnswerSearch(validateSearchInput);
