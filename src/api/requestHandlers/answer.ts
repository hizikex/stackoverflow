...
export const createAnswer: RequestHandler = async ( req, res, next): Promise<void> => {
    try {
        const validatedAnswerData = await validateAnswerCreation(req.body);
        const response = await answerController.processAnswer(res.locals.user, req.params.questionId, validatedAnswerData);
