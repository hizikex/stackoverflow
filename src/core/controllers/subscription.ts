...
export const processQuestionUnsubscription = async (
    userId: number,
    questionId: number
  ): Promise<QuestionUnsubscriptionResponse> => {
    const user = await User.findOne({ where: { id: userId } });
    ...
    await Subscription.destroy({ where: {user_id: user.id, question_id: question.id} });
      ...
      html: `
          ...
          <body>
              <div class="container">
                  <h2>Subscription Confirmation</h2>
                  <p>Hi <span class="username">${user.username}</span>!</p>
                  <p>You have successfully unsubscribed question with ID: <span class="question-id">${question.id}</span>.</p>
              </div>
          </body>
          </html>        
    `,
    ...
    try {
      await sendMail(options);
      logger.info("Unsubscription to question was successful");
    } catch (error) {
      logger.error("Mail not sent");
      throw new ConflictError("Mail not sent", null);
    }
    ...
    return {
      message: `Hi ${user.username}! You have successfully unsubscription question with id: ${question.id}`,
    };
  };
